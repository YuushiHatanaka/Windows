...
namespace Common.Data.Json
{
    ...
    public class JsonParser
    {
        ...
        public JsonParser()
        {
            Logger.Start("JsonParser::JsonParser()");
            Logger.Stop("JsonParser::JsonParser()");
        }
        ...
        {
            Logger.Start("JsonParser::JsonParser(object)");

            // 初期化
            this.m_user_data = userData;

            Logger.Stop("JsonParser::JsonParser(object)");
        }
        #endregion
        ...
        {
            Logger.Start("JsonParser::is_string(string, ref string)");
            ...
            if (Regex.IsMatch(_trim_str, "^\\\".*\\\"$"))
            {
                ...
                // 一致
                Logger.Stop("JsonParser::is_string(string, ref string)");
                return true;
            }
        ...
        {
            Logger.Start("JsonParser::is_number(string)");

            // 実数正規表現
            if (Regex.IsMatch(str, @"[\-]*[0-9]*\.[0-9]*$"))
            {
                // 一致
                Logger.Stop("JsonParser::is_number(string)");
                return true;
            }
            // 指数正規表現
        ...
        {
            Logger.Start("JsonParser::initialize_array_index()");
            ...
            Logger.Stop("JsonParser::initialize_array_index()");
        }
        ...
        {
            Logger.Start("JsonParser::update_array_index()");
            ...
            if (_m_array_index_stack_count <= 0)
            {
                ...
                Logger.Error("JsonParser::update_array_index()");
                return false;
            }
            ...
            // 正常終了
            Logger.Stop("JsonParser::update_array_index()");
            return true;
        }
        ...
        {
            Logger.Start("JsonParser::update_array_index(uint)");

            // Arrayインデックス更新
            if (!this.update_array_index())
            {
                // 異常終了
                Logger.Error("JsonParser::update_array_index(uint)");
                return false;
            }
            ...
            // 正常終了
            Logger.Stop("JsonParser::update_array_index(uint)");
            return true;
        }
        ...
        {
            Logger.Start("JsonParser::Parse(string)");
            Logger.Information("buf:[{0}]", buf);
            ...
            {
                // 空行なので処理する必要なし(正常終了)
                Logger.Stop("JsonParser::Parse(string)");
                return true;
            }
            ...
            {
                // 異常終了
                Logger.Error("JsonParser::Parse(string)");
                return false;
            }
        ...
        private bool Parse()
        {
            ...
            if (this.m_strings_queue.Length() == 0)
            {
                ...
                Logger.Stop("JsonParser::Parse()");
                return true;
            }

            // 状態で分岐
            Logger.Information("《遷移状態⇒{0}[{1}]》", this.m_status_stack.Get(), this.m_status_stack.ToString());
                ...
                default:
                    {
                        // 異常終了
                        this.Error(JsonError.SystemError);
                        Logger.Critical("JsonParser::Parse()");
                        return false;
                    }
            }
        }
        ...
        {
            Logger.Start("JsonParser::Parse(StreamReader)");
            ...
            while (stream.Peek() >= 0)
            {
                ...
                {
                    // 異常終了
                    Logger.Error("JsonParser::Parse(StreamReader)");
                    return false;
                }
            }

            // 正常終了
            Logger.Stop("JsonParser::Parse(StreamReader)");
            return true;
        }
        ...
        {
            Logger.Start("JsonParser::Parse(StringReader)");
            ...
            while (stream.Peek() >= 0)
            {
                ...
                {
                    // 異常終了
                    Logger.Error("JsonParser::Parse(StringReader)");
                    return false;
                }
            }

            // 正常終了
            Logger.Stop("JsonParser::Parse(StringReader)");
            return true;
        }
        ...
        {
            Logger.Start("JsonParser::Parse(MemoryStream)");
            ...
            {
                // 異常終了
                Logger.Error("JsonParser::Parse(MemoryStream)");
                return false;
            }

            // 正常終了
            Logger.Stop("JsonParser::Parse(MemoryStream)");
            return true;
        }
        #endregion
        ...
        {
            Logger.Start("JsonParser::find_start_tag()");
            ...
            while (!this.m_strings_queue.Eof())
            {
                ...
                if (_column == "{")
                {
                    ...
                    {
                        // 異常終了
                        Logger.Error("JsonParser::find_start_tag()");
                        return false;
                    }
                    ...
                    // 正常終了
                    Logger.Stop("JsonParser::find_start_tag()");
                    return true;
                }
                // 開始タグ(Array)の場合
                else if (_column == "[")
        ...
        {
            Logger.Start("JsonParser::find_next_state()");
            /////
            /////    // 直前のモード取得
            ...
            /////    }
            /////
            Logger.Error("JsonParser::find_next_state()");
            return false;
        }
        ...
        {
            Logger.Start("JsonParser::find_string(ref string)");
            ...
            while (!this.m_strings_queue.Eof())
            {
                ...
                if (_column == "\"")
                {
                    ...
                    if (!_escape)
                    {
                        ...
                        // 正常終了
                        Logger.Stop("JsonParser::find_string(ref string)");
                        return true;
                    }
                    else
                    {
            ...
            // 該当なし
            Logger.Verbose("JsonParser::find_string(ref string)");
            return false;
        }
        ...
        {
            Logger.Start("JsonParser::find_object_key()");
            ...
            while (!this.m_strings_queue.Eof())
            {
                ...
                if (_column == "}")
                {
                    ...
                    {
                        // 異常終了
                        Logger.Error("JsonParser::find_object_key()");
                        return false;
                    }
                    ...
                    // 正常終了
                    Logger.Stop("JsonParser::find_object_key()");
                    return true;
                }
                // :の場合
                else if (_column == ":")
        ...
        {
            Logger.Start("JsonParser::find_object_next_key()");
            ...
            if (_key_stream.ToString().Length == 0)
            {
                // 構文エラー
                this.Error(JsonError.InvalidSyntax);
                Logger.Error("JsonParser::find_object_next_key()");
                return false;
            }
            ...
            if (this.is_string(_key_stream.ToString(), ref _key))
            {
                ...
                // 正常終了
                Logger.Stop("JsonParser::find_object_next_key()");
                return true;
            }
        ...
        {
            Logger.Start("JsonParser::find_object_start_value()");
            ...
            while (!this.m_strings_queue.Eof())
            {
                ...
                if (_column == ":")
                {
                    ...
                    // Object値開始
                    Logger.Stop("JsonParser::find_object_start_value()");
                    return true;
                }
                break;
            }
            ...
            // 次Value開始タグなし
            this.Error(JsonError.InvalidSyntax);
            Logger.Error("JsonParser::find_object_start_value()");
            return false;
        }
        ...
        {
            Logger.Start("JsonParser::find_object_value()");
            ...
            while (!this.m_strings_queue.Eof())
            {
                ...
                if (_column == "{")
                {
                    ...
                    {
                        // 異常終了
                        Logger.Error("JsonParser::find_object_value()");
                        return false;
                    }
                    ...
                    // 正常終了
                    Logger.Stop("JsonParser::find_object_value()");
                    return true;
                }
                // 終了タグ(Object)の場合
                else if (_column == "}")
        ...
        {
            Logger.Start("JsonParser::find_object_next_start()");
            ...
            while (!this.m_strings_queue.Eof())
            {
                ...
                if (_column == ",")
                {
                    ...
                    {
                        // 異常終了
                        Logger.Error("JsonParser::find_object_next_start()");
                        return false;
                    }
                    ...
                    // 正常終了
                    Logger.Stop("JsonParser::find_object_next_start()");
                    return true;
                }
                break;
            }
            ...
            // 次Objectなし
            Logger.Verbose("JsonParser::find_object_next_start()");
            return false;
        }
        ...
        {
            Logger.Start("JsonParser::terminate_end_object_array_index()");
            ...
            {
                // Arrayではないので処理終了
                Logger.Stop("JsonParser::terminate_end_object_array_index()");
                return true;
            }
            ...
            {
                // 異常終了
                Logger.Error("JsonParser::terminate_end_object_array_index()");
                return false;
            }
        ...
        {
            Logger.Start("JsonParser::get_end_object_next_status()");
            ...
            // 次遷移状態を返却
            Logger.Information("《次遷移状態⇒[{0}]》", this.m_status_stack.ToString(_status));
            Logger.Stop("JsonParser::get_end_object_next_status()");
            return _status;
        }
        ...
        {
            Logger.Start("JsonParser::find_array_value()");
            ...
            while (!this.m_strings_queue.Eof())
            {
                ...
                if (_column == "{")
                {
                    ...
                    {
                        // 異常終了
                        Logger.Error("JsonParser::find_array_value()");
                        return false;
                    }
                    ...
                    // 正常終了
                    Logger.Stop("JsonParser::find_array_value()");
                    return true;
                }
                // 開始タグ(Array)の場合
                else if (_column == "[")
            ...
            switch (_JsonStatus)
            {
                ...
                default:
                    // 異常終了
                    this.Error(JsonError.SystemError);
                    Logger.Critical("JsonParser::find_array_value()");
                    return false;
            }
        }
        ...
        {
            Logger.Start("JsonParser::find_array_end_tag()");
            ...
            while (!this.m_strings_queue.Eof())
            {
                ...
                if (_column == "]")
                {
                    ...
                    {
                        // 異常終了
                        Logger.Error("JsonParser::find_array_end_tag()");
                        return false;
                    }
                    ...
                    // 正常終了
                    Logger.Stop("JsonParser::find_array_end_tag()");
                    return true;
                }
                // 次Value存在有の場合
                else if (_column == ",")
        ...
        {
            Logger.Start("JsonParser::find_array_next_start()");
            ...
            while (!this.m_strings_queue.Eof())
            {
                ...
                if (_column == ",")
                {
                    ...
                    {
                        // 異常終了
                        Logger.Error("JsonParser::find_array_next_start()");
                        return false;
                    }
                    ...
                    // 正常終了
                    Logger.Stop("JsonParser::find_array_next_start()");
                    return true;
                }
                break;
            }
            ...
            // 次Arrayなし
            Logger.Verbose("JsonParser::find_array_next_start()");
            return false;
        }
        ...
        {
            Logger.Start("JsonParser::StartObject()");
            ...
            if (this.OnStartObject != null)
            {
                ...
                {
                    // 異常終了
                    Logger.Error("JsonParser::StartObject()");
                    return false;
                }
            }

            // 正常尾終了
            Logger.Stop("JsonParser::StartObject()");
            return true;
        }
        ...
        {
            Logger.Start("JsonParser::EndObject()");
            ...
            if (this.m_mode_stack.Count() <= 0)
            {
                // 異常終了
                this.Error(JsonError.SystemError);
                Logger.Error("JsonParser::EndObject()");
                return false;
            }
            ...
            // 正常終了
            Logger.Stop("JsonParser::EndObject()");
            return true;
        }
        ...
        {
            Logger.Start("JsonParser::StartArray()");
            ...
            if (this.OnStartArray != null)
            {
                ...
                {
                    // 異常終了
                    Logger.Error("JsonParser::StartArray()");
                    return false;
                }
            }

            // 正常終了
            Logger.Stop("JsonParser::StartArray()");
            return true;
        }
        ...
        {
            Logger.Start("JsonParser::EndArray()");
            ...
            if (this.m_mode_stack.Count() <= 0)
            {
                // 異常終了
                this.Error(JsonError.SystemError);
                Logger.Error("JsonParser::EndArray()");
                return false;
            }
            ...
            // 正常終了
            Logger.Stop("JsonParser::EndArray()");
            return true;
        }
        ...
        {
            Logger.Start("JsonParser::StartElement(uint, string)");
            ...
            if (this.OnStartElement != null)
            {
                ...
                {
                    // 異常終了
                    Logger.Error("JsonParser::StartElement(uint, string)");
                    return false;
                }
            }

            // 正常終了
            Logger.Stop("JsonParser::StartElement(uint, string)");
            return true;
        }
        ...
        {
            Logger.Start("JsonParser::EndElement(uint, string)");

            // キー数を判定
            if (this.m_key_stack.Count() <= 0)
            {
                // 正常終了
                Logger.Stop("JsonParser::EndElement(uint, string)");
                return true;
            }
            ...
            if (this.OnEndElement != null)
            {
                ...
                {
                    // 異常終了
                    Logger.Error("JsonParser::EndElement(uint, string)");
                    return false;
                }
            }
        ...
        {
            Logger.Start("JsonParser::Key(uint, string)");

            // TODO:未実装

            // 正常終了
            Logger.Stop("JsonParser::Key(uint, string)");
            return true;
        }
        ...
        {
            Logger.Start("JsonParser::Value(uint, string)");
            ...
            if (this.OnValue != null)
            {
                ...
                {
                    // 異常終了
                    Logger.Error("JsonParser::Value(uint, string)");
                    return false;
                }
            }
            ...
            if (value == "null")
            {
                ...
                // 正常終了
                Logger.Stop("JsonParser::Value(uint, string)");
                return true;
            }
            // 値判定(bool)
        ...
        {
            Logger.Start("JsonParser::String(uint, string)");
            ...
            if (this.OnString != null)
            {
                ...
                {
                    // 異常終了
                    Logger.Error("JsonParser::String(uint, string)");
                    return false;
                }
            }

            // 正常終了
            Logger.Stop("JsonParser::String(uint, string)");
            return true;
        }
        ...
        {
            Logger.Start("JsonParser::Null(uint)");
            ...
            if (this.OnNull != null)
            {
                ...
                {
                    // 異常終了
                    Logger.Error("JsonParser::Null(uint)");
                    return false;
                }
            }

            // 正常終了
            Logger.Stop("JsonParser::Null(uint)");
            return true;
        }
        ...
        {
            Logger.Start("JsonParser::Bool(uint, string)");
            ...
            if (this.OnBool != null)
            {
                ...
                {
                    // 異常終了
                    Logger.Error("JsonParser::Bool(uint, string)");
                    return false;
                }
            }

            // 正常終了
            Logger.Stop("JsonParser::Bool(uint, string)");
            return true;
        }
        ...
        {
            Logger.Start("JsonParser::Error(JsonError)");
            ...
            Logger.Stop("JsonParser::Error(JsonError)");
        }
    }
}