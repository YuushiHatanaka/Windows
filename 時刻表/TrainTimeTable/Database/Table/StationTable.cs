...
namespace TrainTimeTable.Database.Table
{
    ...
    public class StationTable : TableListCore<StationProperties, StationProperty>
    {
        ...
        /// <returns></returns>
        protected override StationProperties GetRemoveKeys(StationProperties srcProperties, StationProperties dstProperties)
        {
            // ロギング
            Logger.Debug("=>>>> StationProperties::GetRemoveKeys(StationProperties, StationProperties)");
            Logger.DebugFormat("srcProperties:[{0}]", srcProperties);
            Logger.DebugFormat("dstProperties:[{0}]", dstProperties);

            // 結果オブジェクト生成
            StationProperties result = new StationProperties();
        ...
        protected override void Remove(StationProperties properties)
        {
            // ロギング
            Logger.Debug("=>>>> StationTable::Rebuilding(StationProperties)");
