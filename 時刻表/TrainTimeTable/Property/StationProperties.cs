...
namespace TrainTimeTable.Property
{
    ...
    public class StationProperties : List<StationProperty>
    {
        ...
        /// <returns></returns>
        public StationProperty GetBeforeStation(DirectionType type, StationSequenceProperties sequences, StationProperty property)
        {
            // ロギング
            Logger.Debug("=>>>> StationProperties::GetBeforeStation(DirectionType, StationSequenceProperties, StationProperty)");
            ...
            StationSequenceProperty stationSequenceProperty  = sequences.Find(s => s.Name == property.Name);
            ...
            StationSequenceProperty nextSequenceProperty = sequences.Find(s => s.Seq == stationSequenceNo);

            // 結果設定
            if (nextSequenceProperty != null)
            {
                result = Find(s => s.Name == nextSequenceProperty.Name);
            }
        ...
        /// <param name="property"></param>
        /// <returns></returns>
        public StationProperty GetAfterStation(DirectionType type, StationSequenceProperties sequences, StationProperty property)
        {
            // ロギング
            Logger.Debug("=>>>> StationProperties::GetAfterStation(DirectionType, StationSequenceProperties, StationProperty)");
            Logger.DebugFormat("type     :[{0}]", type);
            Logger.DebugFormat("sequences:[{0}]", sequences);
            Logger.DebugFormat("property :[{0}]", property);
